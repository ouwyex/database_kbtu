...
SELECT UPPER(first_name || ' ' || last_name) AS upper_name, LENGTH(last_name) AS last_name_length, SUBSTRING(email FROM 1 FOR 3) AS email_prefix FROM employees;

SELECT first_name || ' ' || last_name AS full_name, salary AS annual_salary, ROUND(salary / 12, 2) AS monthly_salary, salary * 0.10 AS raise_amount FROM employees;
...
SELECT * FROM employees WHERE employee_id NOT IN (SELECT employee_id FROM assignments);
...
SELECT e.first_name || ' ' || e.last_name AS full_name, e.department, AVG(a.hours_worked) AS avg_hours, RANK() OVER (PARTITION BY e.department ORDER BY e.salary DESC) AS salary_rank FROM employees e LEFT JOIN assignments a ON e.employee_id = a.employee_id GROUP BY e.employee_id, e.first_name, e.last_name, e.department, e.salary;

SELECT p.project_name, SUM(a.hours_worked) AS total_hours, COUNT(DISTINCT a.employee_id) AS num_employees FROM projects p JOIN assignments a ON p.project_id = a.project_id GROUP BY p.project_name HAVING SUM(a.hours_worked) > 150;

SELECT department, COUNT(*) AS total_employees, AVG(salary) AS avg_salary, (SELECT first_name || ' ' || last_name FROM employees e2 WHERE e2.department = e.department ORDER BY salary DESC LIMIT 1) AS highest_paid_employee, GREATEST(MAX(salary), AVG(salary)) AS greatest_value, LEAST(MIN(salary), AVG(salary)) AS least_value FROM employees e GROUP BY department;
